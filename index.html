...
<title>TikTok Demo Funcional iOS</title>
<style>
:root{
...
body{margin:0;background:var(--bg);color:var(--text);display:flex;justify-content:center;padding-top:env(safe-area-inset-top);padding-bottom:env(safe-area-inset-bottom);position:relative;}
.app{width:100%;max-width:430px;min-height:100vh;padding:16px;perspective:800px;}
...
.overlay{position:fixed;inset:0;background:rgba(0,0,0,.35);backdrop-filter: blur(18px) saturate(180%);-webkit-backdrop-filter: blur(18px) saturate(180%);display:flex;align-items:flex-end;justify-content:center;animation:fade .15s;}
...
.coin-fly{position:fixed;width:32px;height:32px;z-index:9999;pointer-events:none;transform-style:preserve-3d;will-change:transform;}
...
<audio id="coinSound" src="https://freesound.org/data/previews/341/341695_624037-lq.mp3"></audio>
...
user.addEventListener('input', validarUsuario);

function validarUsuario(){
  const u=user.value.trim(); haptic();
  const formato=/^@[a-zA-Z0-9._-]{1,30}$/;
  if(!formato.test(u)){
    error.style.display="block"; avatar.innerText="U"; saldo.innerText="0"; hist.innerHTML=""; bloquear(true); 
    return;
  }
  error.style.display="none"; 
  avatar.innerText=u[1].toUpperCase();
  if(!USUARIOS[u]) USUARIOS[u]=0; // Crear usuario dinámicamente
  saldo.innerText=USUARIOS[u]; 
  historial=[]; 
  hist.innerHTML=""; 
  bloquear(false);
}
...
function confirmar(pkgEl,c){
  ...
  o.innerHTML=`<div class="modal">
    ...
    <button class="confirm pulse" onclick="animarMonedas(pkgEl);pagar('${u}',${c});o.remove()">Confirmar</button>
...
function crearMoneda(pkgEl){
  ...
  coin.innerHTML=`<svg viewBox="0 0 64 64">
    <circle cx="32" cy="32" r="30" fill="url(#grad)"/>
    <text x="32" y="40" font-size="28" text-anchor="middle" fill="#000" font-weight="700" font-family="Arial">🎵</text>
    <defs><radialGradient id="grad" cx="0.3" cy="0.3" r="0.7"><stop offset="0%" stop-color="#FFD700"/><stop offset="100%" stop-color="#FFA500"/></radialGradient></defs>
  </svg>`;
  ...
  requestAnimationFrame(()=>{
    coin.style.transition="transform 0.8s ease-out, opacity 0.8s ease-out";
    coin.style.transform=`translate3d(${dx}px,${dy}px,${dz}px) rotateX(${Math.random()*360}deg) rotateY(${Math.random()*360}deg) rotateZ(${Math.random()*360}deg) scale(0.5)`;
    coin.style.opacity=0;
    coinSound.currentTime=0; coinSound.play();
    crearParticulas(rect.left+rect.width/2, rect.top);
  });
...
function crearParticulas(x,y){
  for(let i=0;i<5;i++){
    ...
    requestAnimationFrame(()=>{p.style.transform=`translate3d(${dx}px,${dy}px,${dz}px)`; p.style.opacity=0;});
...
function pagar(u,c){
  haptic(); 
  const base=USUARIOS[u]; 
  const target=base+c; 
  USUARIOS[u]=target;
  let current=base; 
  const step=Math.ceil(c/10);
  const interval=setInterval(()=>{
    current+=step; 
    if(current>=target) current=target;
    saldo.innerText=current; 
    saldo.style.transform='scale(1.2)';
    setTimeout(()=>saldo.style.transform='scale(1)',100);
  ...
  historial.forEach(h=>{
    ...
    div.className="item"; 
    div.innerHTML=`<span>+${h.c}</span><span>${h.t}</span>`; 
